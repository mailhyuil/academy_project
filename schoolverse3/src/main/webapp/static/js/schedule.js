...
table.addEventListener("mouseover", (e) => {
  if (e.target.className == "classes") {
    ...
    for (let schedule of splitSchedules) {
      // 수업 스케줄과 td의 날짜를 비교하여 컬러링
      for (let t of td) {
        if (
          t.parentNode.dataset.day == schedule.split("_")[0] &&
...
table.addEventListener("drop", (e) => {
  ...
  /* 스케줄에 집어넣기 */
  if (
    e.target.className === "dates" && // js에서 class 추가했음
    e.target.parentNode.className === "days"
  ) {
    // js에서 class 추가했음
    ...
    for (let i = 0; i < schedule.length; i++) {
      ...
      for (let t of td) {
        if (t.dataset.date === date && t.parentNode.dataset.day === day) {
